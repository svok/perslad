...
# from openinference.instrumentation.langgraph import LangGraphInstrumentor # Not available on PyPI yet
...
class MetricsManager:
    ...
    def initialize(self, service_name: str = "perslad-agent") -> None:
        ...
        otlp_endpoint = os.getenv("OTLP_ENDPOINT", "http://phoenix:6006")
        api_key = os.getenv("OTLP_API_KEY")
        
        # OpenTelemetry is enabled if endpoint is configured
        if otlp_endpoint:
            try:
                ...
                # Ensure endpoint ends with /v1/traces for HTTP exporter
                if not otlp_endpoint.endswith("/v1/traces"):
                    otlp_endpoint = f"{otlp_endpoint.rstrip('/')}/v1/traces"
                
                logger.info(f"Initializing OTLP HTTP exporter to {otlp_endpoint}")
                ...
                # Instrument LangChain (covers LangGraph as well) - optional
