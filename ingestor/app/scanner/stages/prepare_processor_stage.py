...
from ingestor.app.scanner.fileinfo import FileInfo
from ingestor.app.scanner.stages.pipeline_stage import PipelineStage
...
class PrepareProcessorStage(PipelineStage):
    ...
    async def process(self, file_path: Path) -> Optional[FileInfo]:
        ...
        try:
            ...
            stat_future = loop.run_in_executor(self.executor, file_path.stat)
            checksum_future = loop.run_in_executor(
                self.executor, self._calculate_checksum, file_path
            )

            stat, checksum = await asyncio.gather(stat_future, checksum_future)
